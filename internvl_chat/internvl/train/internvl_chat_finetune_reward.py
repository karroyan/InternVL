...
def compute_classfication_metric(pred):
    predictions, labels = pred
    label = labels[-1].reshape(predictions[0].shape[0], 1) # (400, 32)
    accuracy_dict = {}
    accuracy_mean = 0
    ...
    single_label = label.squeeze()
    accuracy = np.mean(predicted_classes == single_label)

    accuracy_dict[f'acc'] = accuracy
...
def main():
    ...
    trainer = Trainer(
        ...
        compute_metrics=compute_classfication_metric,
